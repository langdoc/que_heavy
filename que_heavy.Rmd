...
words %>% filter(str_detect(content, "army"))
def %>% filter(str_detect(content, "army"))

words

def_start <- def %>% rename(utterance = content) %>% filter() %>%
                  select(-annot_id, -ref_id, -tier_id, -type, -time_slot_1, -time_slot_2, -time_end, -participant)



left_join(words, ) %>% View()
